...
export default function Overly() {
    ...
    // 🎞️ Ensure autoplay works on mobile (requires muted + playsInline)
    useEffect(() => {
        if (videoRef.current) {
            ...
            if (playPromise !== undefined) {
                playPromise.catch(() => {
                    // If autoplay fails (e.g. iOS), just ignore
                });
            }
        }
    }, [currentVideo]);
    return (
        <div className="relative w-full h-150 overflow-hidden overflow-x-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center gap-20 justify-center h-full text-center text-gray-800">
                ...
                <button className="mt-6 max-sm:ml-[-20px] bg-orange-400 text-white  font-semibold text-[20px] w-60 py-3 rounded-xl shadow-md hover:scale-105 hover:shadow-lg transition-all duration-300" onClick={() => {
