...
export default function Overly() {
    const videos = ["/sushi1.mp4", "/sushi2.mp4", "/sushi3.mp4"]; // your video files
    ...
    useEffect(() => {
        ...
        }, 8000); // change every 8 seconds (you can adjust)
    ...
    return (
        <div className="relative w-full h-150 overflow-hidden overflow-x-hidden">
            {/* 🎥 Background video */}
            <video
                key={currentVideo} // this restarts video when it changes
                src={videos[currentVideo]}
                autoPlay
                muted
                playsInline
                loop={false} // no loop, since we’re manually changing videos
            ...
            <div className='flex flex-col items-center gap-10 max-sm:mt-[-80px]'>
                <h1 className="text-5xl max-sm:w-90 max-sm:ml-[-15px] max-sm:text-xl md:text-5xl font-bold text-white drop-shadow-[0_3px_3px_rgba(0,0,0,0.5)]">“Every detail matters.
                    From sea to plate, sushi is not just food — it’s art.”</h1>
                <p className=" max-sm:w-90 max-sm:ml-[-15px] text-xl text-gray-200 mt-4 max-w-lg drop-shadow-[0_3px_3px_rgba(0,0,0,0.5)]">
                    ...
                    authentique où la fraîcheur,
                    la qualité et la passion se rencontrent dans chaque création.
                </p>
                ...
                <button className="mt-6 max-sm:ml-[-20px] bg-orange-400 text-white  font-semibold text-[20px] w-60 py-3 rounded-xl shadow-md hover:scale-105 hover:shadow-lg transition-all duration-300">
                    メニューを見る
                </button>
